...
public enum Gpg45Profile {
    ...
    M1C("M1C", new Gpg45Scores(EV_32, 1, 2, 2)),
    M1B("M1B", new Gpg45Scores(EV_33, 0, 0, 3)),
    ...
    H2D("H2D", new Gpg45Scores(EV_33, EV_32, 0, 1, 3)),
    ...
    /**
     * Checks if that the provided {@code Gpg45Scores} matches the {@code Gpg45Scores} for this profile.
     *
     * @param target The {@code Gpg45Scores} to compare.
     * @return {@code true} if the elements in the provided {@code Gpg45Scores} are all equal or greater than the profile's `{@code Gpg45Scores}.
     */
    public boolean match(Gpg45Scores target){
        return matchEvidence(target) &&
                (scores.activity() <= target.activity()) &&
                (scores.fraud() <= target.fraud()) &&
                (scores.verification() <= target.verification());
    }

    private boolean matchEvidence(Gpg45Scores target){
