...
/**
 * Gpg45Scores is a collection of values that represent the scores gathered when assessing identity proofing using GPG 45.
 *
 * Objects if this class are immutable.
 */
public class Gpg45Scores implements Comparable<Gpg45Scores>{

    public static final int STRENGTH_MAX = 4;
    public static final int VALIDITY_MAX = 4;
    public static final int ACTIVITY_MAX = 4;
    public static final int FRAUD_MAX = 3;
    public static final int VERIFICATION_MAX = 4;
    ...
    public Gpg45Scores(Evidence evidence1, Evidence evidence2, int activity, int fraud, int verification){
        this(Arrays.asList(evidence1, evidence2), activity, fraud, verification);
    }

    public Gpg45Scores(Evidence evidence1, Evidence evidence2, Evidence evidence3, int activity, int fraud, int verification){
    ...
    public Gpg45Scores(List<Evidence> evidence, int activity, int fraud, int verification){
        ...
        this.fraud = fraud;
        this.verification = verification;
        evidence.sort(Comparator.comparing(Evidence::strength).reversed());
        this.evidences = List.copyOf(evidence);
    }
    ...
    public Evidence getEvidence(int index){
        return getEvidence(index, false);
    }

    public Evidence getEvidence(int index, boolean safe){
    ...
    public boolean equals(Object other) {
        ...
        return activity == that.activity && fraud == that.fraud && verification == that.verification && evidences.equals(that.evidences);
    }
    ...
    private List<Gpg45Scores.Evidence> diffEvidence(Gpg45Scores target){
        ...
        var evidenceDiff = new ArrayList<Gpg45Scores.Evidence>();
        var maxEvidence = Math.max(evidences.size(), target.evidences().size());

        for(int i = 0; i < maxEvidence; i++){
            var sourceEvidence = getEvidence(i, true);
            var targetEvidence = target.getEvidence(i, true);
